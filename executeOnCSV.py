...
def main(filename, desc_column_name, outputName, selectedModel, topPreds):
    ...
    if selectedModel == 0:
        ...
        # link pred is not available
        print("Model not yet available")
    elif selectedModel == 4:
        graph_model = graph.model(epochs=1, test_file = "intermediate.csv", totalClusters= 20, ground_truth_file = "foo.csv", write_ground_truth= False)
        graph_model.train()
        graph_model_out = graph_model.test(topPreds)
