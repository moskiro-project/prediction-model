...
def find_lda(id2word, corpus):
    ...
    for i in var:
        ...
        if lda_model.log_perplexity(corpus) < min_per[0]:
            min_per = (lda_model.log_perplexity(corpus),i)
            lda_per = lda_model
    ...
    return (max_cohe,lda_cohe),(min_per,lda_per),var,res
...
if __name__ ==  '__main__':
    ...
    coherence,perplexity, num_topics, results= find_lda(id2word,corpus)
    save_lda(perplexity[1], df, "perp")
    ...
    df = pd.read_csv("naukri_data_science_jobs_india_cleaned_clusterd.csv", converters={"Skills/Description": pd.eval})
